...
void makeMacAddress(struct ethhdr* ether_header, char *macAddrToString) {
...
void processTCPOptions(uint8_t* seekToOptions, tcphdr* tcp_header) {
	...
	if (tcp_header->th_off * 4 > 20) {
		while (*seekToOptions != 0) {
			tcp_option_t* _seekToOptions = (tcp_option_t*) seekToOptions;
			if (_seekToOptions->kind == 1 /* NOP */) {
				char buffer[50];
				sprintf(buffer, "%u (0x%02x)", _seekToOptions->kind,
						_seekToOptions->kind);
				storeInMap(OptionsMap, buffer);
				seekToOptions++;  // NOP is one byte;
				count++;
			} else if (_seekToOptions->kind != 1 /* Others */) {
				char buffer[50];
				sprintf(buffer, "%u (0x%02x)", _seekToOptions->kind,
						_seekToOptions->kind);
				storeInMap(OptionsMap, buffer);
				seekToOptions += _seekToOptions->size;
				count += _seekToOptions->size;
			}
...
void my_callback(u_char *packetinfo, const struct pcap_pkthdr* header,
	...
	char destaddrmac[18] = "";
	makeMacAddress(ether_header, destaddrmac);
	// src mac address to string
	char srcaddrmac[18] = "";
	makeMacAddress(ether_header, srcaddrmac);
	...
	if (ntohs(ether_header->h_proto) == ETHERTYPE_IP) {
		...
		if (ip_header->protocol == IPPROTO_TCP) {
			...
			struct tcphdr* tcp_header = (struct tcphdr*) (packet + ETHER_HDR_LEN
					+ sizeof(iphdr));
//Source TCP portstcp_header
			cout << "Printttttttttttttttttttttttttt out" << endl;
...
//Counting ICMP
		...
		makeMacAddress(ether_header, arpMacToString);
